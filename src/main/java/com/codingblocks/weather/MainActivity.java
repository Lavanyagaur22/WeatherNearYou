    ...
    private void makeNetworkCall(String url) {
        ...
        okHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                Gson gson = new Gson();
                finalclass finalclass = gson.fromJson(res, finalclass.class);

                ArrayList<result> list = finalclass.getList();

//                Log.e("TAG", "onResponse: "+list.get(i).getMain().getTemp());

                for (int i = 0; i < list.size(); i++) {

                    Log.e("TAG", "onResponse: "+list.size());
                    Log.e("TAG", "onResponse: --- "+list.get(i));
